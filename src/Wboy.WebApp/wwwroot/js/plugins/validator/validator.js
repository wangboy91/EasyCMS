    ...
    $(this).find("[isvalid=yes]").each(function () {
        ...
        if (checkexpession != undefined) {
            ...
            if ($(this).attr("type") == "file") {
                value = $(this).next().find("input").val();
            }
            switch (checkexpession) {
                ...
                case "Password":
                    {
                        if (!isPassword(value)) {
                            Validatemsg = errormsg + "密码位数必需是6-12！\n";
                            Validateflag = false;
                            ValidationMessage($(this), Validatemsg); return false;
                        }
                        break;
...
function ValidationMessage(obj, Validatemsg) {
    try {
        ...
        if (obj.attr("type") == "select") {
            $poptip_error = $('<div class="poptip"><span class="poptip-arrow poptip-arrow-top"><em>◆</em></span>' + Validatemsg + '</div>').css("left", obj.parent().offset().left + 'px').css("top", obj.parent().offset().top + obj.parent().height() + 5 + 'px');
        }
        ...
        if (obj.attr("type") == "file") {
            obj.next().find("input").change(function () {
...
function showMessage(obj, msg) {
    ...
    if (obj.attr("type") == "select" || obj.attr("type") == "file") {
        $poptip_error = $('<div class="poptip"><span class="poptip-arrow poptip-arrow-top"><em>◆</em></span>' + msg + '</div>').css("left", obj.parent().offset().left + 'px').css("top", obj.parent().offset().top + obj.parent().height() + 5 + 'px');
    }
    $('body').append($poptip_error);
    if (obj.hasClass('form-control') || obj.hasClass('ui-select')) {
        obj.parent().addClass('has-error');
