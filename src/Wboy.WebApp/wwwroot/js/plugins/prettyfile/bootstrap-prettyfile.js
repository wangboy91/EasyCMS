/*
 * jQuery and Bootsrap3 Plugin prettyFile
 *
 * version 2.0, Jan 20th, 2014
 * by episage, sujin2f
 * Git repository : https://github.com/episage/bootstrap-3-pretty-file-upload
 */
(function ($) {
    $.fn.extend({
        prettyFile: function (options) {
            var defaults = {
                text: "选择文件",
                url: "",
                isValidator: false,
                callback: null
            };

            var options = $.extend(defaults, options);
            var plugin = this;

            function make_form($el, text) {
                $el.wrap('<div></div>');
                $el.hide();
                if (options.isValidator) {
                    $el.after('\
				<div class="input-append input-group"">\
                    <input class="input-large form-control" type="text" isvalid="yes" checkexpession="Uri" autocomplete="off">\
					<span class="input-group-btn">\
						<a class="btn btn-white">' + text + '</a>\
					</span>\
				</div>\
				' );
                } else {
                    $el.after('\
				<div class="input-append input-group"">\
                    <input class="input-large form-control" type="text" autocomplete="off">\
					<span class="input-group-btn">\
						<a class="btn btn-white">' + text + '</a>\
					</span>\
				</div>\
				' );
                }


                return $el.parent();
            };

            function bind_change($wrap, multiple) {
                $wrap.find('input[type="file"]').change(function () {
                    // When original file input changes, get its value, show it in the fake input
                    var files = $(this)[0].files,
                        info = '';

                    if (files.length == 0)
                        return false;

                    if (!multiple || files.length == 1) {
                        //var path = $( this ).val().split('\\');
                        //info = path[path.length - 1];
                        var data = new FormData();
                        data.append("file", files[0]); //加入文件对象
                        $.ajax({
                            url: options.url,
                            data: data,
                            type: "Post",
                            dataType: "json",
                            cache: false,//上传文件无需缓存
                            processData: false,//用于对data参数进行序列化处理 这里必须false
                            contentType: false, //必须
                            success: function (result) {
                                if (result.type == "1") {
                                    $wrap.find('.input-append input').val(result.message);
                                    dialogMsg("上传成功", 1);
                                    options.callback(result.message);
                                }

                            }, error: function () {
                                alert("上传失败！");
                                $("#imgWait").hide();
                            }
                        });
                    } else if (files.length > 1) {
                        // Display number of selected files instead of filenames
                        info = "已选择了" + files.length + ' 个文件';
                    }

                });
            };

            function bind_button($wrap, multiple) {
                $wrap.find('.input-append a').click(function (e) {
                    e.preventDefault();
                    $wrap.find('input[type="file"]').click();
                });
            };

            return plugin.each(function () {
                $this = $(this);
                if ($this) {
                    var multiple = $this.attr('multiple');
                    $wrap = make_form($this, options.text);
                    bind_change($wrap, multiple);
                    bind_button($wrap);
                }
            });
        }
    });
}(jQuery));

