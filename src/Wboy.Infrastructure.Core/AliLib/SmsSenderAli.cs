...
using Wboy.Infrastructure.Core.Message;
...
namespace Wboy.Infrastructure.Core.AliLib
{
    ...
    /// 阿里云发送短信验证码
    /// </summary>
    public class SmsSenderAli : ISmsSender
    {
        ...
        {
            var verCode = _cache.Get<VerificationCode>(cacheKey);
            if (verCode != null)
            {
                if ((DateTime.Now - verCode.SendTime).TotalSeconds < 600)
                {
                    throw new NeedToShowFrontException("验证码已发送！请不要重复获取");
                }
            }
            string code = CommonExtension.RandomCode(6);
            ...
            {
                LoggerFactory.CreateLoger().Warning("SmsSendError", ex.Message);
                switch (ex.ErrorCode)
                {
                    case "InvalidDayuStatus.Malformed":
                        throw new NeedToShowFrontException("账户短信开通状态不正确");
                    case "InvalidSignName.Malformed":
                        throw new NeedToShowFrontException("短信签名不正确");
                    case "InvalidTemplateCode.Malformed":
                        throw new NeedToShowFrontException("模板状态不正确");
                    case "InvalidRecNum.Malformed":
                        throw new NeedToShowFrontException("验证码发送失败！手机号不正确");
                    case "InvalidParamString.Malformed":
                        throw new NeedToShowFrontException("短信模板中变量不是json格式");
                    case "InvalidParamStringTemplate.Malformed":
                        throw new NeedToShowFrontException("短信模板中变量与模板内容不匹配");
                    case "InvalidSendSms":
                        //throw new NeedToShowFrontException("验证码发送失败！触发业务流控");
                        throw new NeedToShowFrontException("验证码发送失败！请求太频繁");
                    case "InvalidDayu.Malformed":
                        throw new NeedToShowFrontException("变量不能是url，可以将变量固化在模板中");
                }
                throw new NeedToShowFrontException("验证码发送失败！请稍后再试");
            }
            catch (Exception ex)
            {
                LoggerFactory.CreateLoger().Warning("SmsSendError", ex.Message);
